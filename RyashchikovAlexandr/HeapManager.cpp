...
CHeapManager::~CHeapManager()
{
	...
	for (auto allocatedBlock : allocatedBlocks) {
		std::cout << "address: [" << allocatedBlock.first << "] size: " << allocatedBlock.second << std::endl;
	}
}
...
{
	int realSize = roundToMultiple(size, minBlockSize);
	...
	allocatedBlocks[foundBlock.second] = realSize;
